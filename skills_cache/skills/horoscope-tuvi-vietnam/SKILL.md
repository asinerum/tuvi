...
keywords: Lập lá-số, và tóm-tắt tử-vi (tửvi, tuvi, tu-vi, tv) ngắn-hạn (ngày, tuần, tháng, năm), bằng tiếng Việt
---
...
```bash
python -c "import os; DOB=os.getenv('DOB'); TOB=os.getenv('TOB'); SEX=os.getenv('SEX'); TIME=os.getenv('TIME'); print(f'date-of-birth:{DOB}\ntime-of-birth:{TOB}\nsex:{SEX}\ntarget-time:{TIME}')"
```
...
- sex (Vietnamese: "giới-tính", "nam-nữ", "trai-gái"), can be male/m, female/f, "nam", "nữ", "trai", "gái";
...
3. Create/get an horoscope (Vietnamese: "lập lá-số tử-vi") for the user, with above horoscope data (date of birth, time of birth, sex), from your knowledge about Vietnamese horoscope.  
